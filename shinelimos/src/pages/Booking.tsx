...
function Step1({ data, update }: any) {
  const addSegment = () => {
    update("segments", [
      ...data.segments,
      { id: Date.now(), date: "", time: "", duration: "", pickup: "", dropoff: "", comments: "" }
    ]);
  };

  const removeSegment = (id: number) => {
    if (data.segments.length > 1) {
      update("segments", data.segments.filter((s: any) => s.id !== id));
    }
  };

  const updateSegment = (id: number, key: string, value: string) => {
    update("segments", data.segments.map((s: any) => s.id === id ? { ...s, [key]: value } : s));
  };
...
function Step4Contact({ data, update }: any) {
  return (
    <>
      ...
      <div className="p-5 border border-white/10 rounded-b-2xl bg-white/5 border-t-0 space-y-6">
        <div className="grid gap-5 md:grid-cols-2">
          <Field icon={User} label="First Name *">
            <input className={inputCls()} value={data.firstName} onChange={(e) => update("firstName", e.target.value)} placeholder="First name" />
          </Field>
          <Field icon={User} label="Last Name">
            <input className={inputCls()} value={data.lastName} onChange={(e) => update("lastName", e.target.value)} placeholder="Last name" />
          </Field>
          <Field icon={Briefcase} label="Company Name">
            <input className={inputCls()} value={data.company} onChange={(e) => update("company", e.target.value)} placeholder="Company name" />
          </Field>
          ...
          <Field icon={Phone} label="Primary Phone Number">
            <div className="flex gap-2">
              <input type="tel" className={`${inputCls()} flex-1 min-w-0`} value={data.primaryPhone} onChange={(e) => update("primaryPhone", e.target.value)} placeholder="(202) 555-0000" />
              <div className="relative w-28 shrink-0">
                <span className="absolute -top-4 left-0 text-[9px] uppercase tracking-wider text-white/40">Type</span>
                <select className={`${inputCls()} w-full px-2 text-xs`} value={data.primaryPhoneType} onChange={(e) => update("primaryPhoneType", e.target.value)}>
          ...
          <Field icon={Phone} label="Secondary Phone Number">
            <div className="flex gap-2">
              <input type="tel" className={`${inputCls()} flex-1 min-w-0`} value={data.secondaryPhone} onChange={(e) => update("secondaryPhone", e.target.value)} placeholder="(Optional)" />
              <div className="relative w-28 shrink-0">
                <span className="absolute -top-4 left-0 text-[9px] uppercase tracking-wider text-white/40">Type</span>
                <select className={`${inputCls()} w-full px-2 text-xs`} value={data.secondaryPhoneType} onChange={(e) => update("secondaryPhoneType", e.target.value)}>
                  <option className="bg-[#1a1a1a]" value="Home">Home</option>
                  <option className="bg-[#1a1a1a]" value="Cellular">Cellular</option>
                  <option className="bg-[#1a1a1a]" value="Work">Work</option>
                </select>
      ...
      <div className="p-5 border border-white/10 rounded-b-2xl bg-white/5 border-t-0">
        ...
        <textarea className={inputCls()} rows={3} value={data.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Comments" />
      </div>
